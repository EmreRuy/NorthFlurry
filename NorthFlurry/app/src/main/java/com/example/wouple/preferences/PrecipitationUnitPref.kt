...
object PrecipitationUnitPref {
    private const val key = "percipitationUnitKey"
    fun getPrecipitationUnit(context: Context): PrecipitationUnit {
        val sharedPref = context.getSharedPreferences("preferences", Context.MODE_PRIVATE) ?: return PrecipitationUnit.MM
        return sharedPref.getString(key, PrecipitationUnit.MM.name)?.let { PrecipitationUnit.valueOf(it) } ?: PrecipitationUnit.MM
    }
